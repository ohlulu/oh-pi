---
description: "CEO/founder-mode plan review. Rethink the problem, find the 10-star product, challenge premises. Three modes: SCOPE EXPANSION (dream big), HOLD SCOPE (maximum rigor), SCOPE REDUCTION (strip to essentials). Use before implementation to pressure-test whether you're building the right thing."
---
...
4. Interactions have edge cases. Every user-visible interaction has edge cases: double-tap, navigate-away-mid-action, slow connection, stale state, back navigation. Map them.
5. Observability is scope, not afterthought. Logging, analytics, and crash reporting are first-class deliverables, not post-launch cleanup items.
6. Diagrams are mandatory. No non-trivial flow goes undiagrammed. ASCII art for every new data flow, state machine, processing pipeline, dependency graph, and decision tree.
7. Everything deferred must be written down. Vague intentions are lies. TODO or it doesn't exist.
...
grep -r "TODO\|FIXME\|HACK\|XXX" --include="*.swift" -l
find . -name "*.swift" -newer Package.swift | head -20  # Recently touched files
```
Then read project docs, TODO lists, and any existing architecture docs. Map:
...
**STOP.** Use `ask_me` once per issue. Do NOT batch. Recommend + WHY. If no issues or fix is obvious, state what you'll do and move on — don't waste a question. Do NOT proceed until user responds.
...
```
  ...
  -------------------------|-----------------------------|-----------------
  ExampleService.fetch()   | API timeout                 | URLError.timedOut
                           | API returns 429             | RateLimitError
                           | API returns malformed JSON  | DecodingError
                           | No network                  | URLError.notConnectedToInternet
  -------------------------|-----------------------------|-----------------

  ERROR TYPE                     | CAUGHT?   | CATCH ACTION           | USER SEES
  -------------------------------|-----------|------------------------|------------------
  URLError.timedOut              | Y         | Retry 2x, then throw   | "Service temporarily unavailable"
  RateLimitError                 | Y         | Backoff + retry         | Nothing (transparent)
  DecodingError                  | N ← GAP   | —                      | Crash ← BAD
  URLError.notConnectedToInternet| Y         | Show offline state     | "No connection" banner
```
...
Evaluate:
* Attack surface expansion. What new attack vectors does this plan introduce? New endpoints, new params, new file paths, new background tasks?
...
* Secrets and credentials. New secrets? In Keychain / env vars, not hardcoded? Rotatable?
* Dependency risk. New packages? Security track record?
* Data classification. PII, payment data, credentials? Handling consistent with existing patterns?
* Injection vectors. Check all applicable.
...
```
  ...
  ---------------------|------------------------|----------|--------
  Form submission      | Double-tap submit      | ?        |
  ...
  Background task      | Task fails after 3 of  | ?        |
                       | 10 items processed     |          |
                       | Task runs twice (dup)  | ?        |
...
* Naming quality. Are new types, methods, and variables named for what they do, not how they do it?
...
```
  ...
  NEW BACKGROUND TASKS / ASYNC WORK:
    [list each]
...
* What type of test covers it? (Unit / Integration / UI)
...
Test pyramid check: Many unit, fewer integration, few UI? Or inverted?
Flakiness risk: Flag any test depending on time, randomness, external services, or ordering.
**STOP.** Use `ask_me` once per issue. Do NOT batch. Recommend + WHY. If no issues or fix is obvious, state what you'll do and move on — don't waste a question. Do NOT proceed until user responds.

### Section 7: Performance Review
Evaluate:
* N+1 queries / redundant fetches. For every new data fetch: is there batching?
* Memory usage. For every new data structure: what's the maximum size in production?
* Caching opportunities. For every expensive computation or external call: should it be cached?
* Background task sizing. For every new task: worst-case payload, runtime, retry behavior?
* Slow paths. Top 3 slowest new codepaths and estimated p99 latency.
* Main thread impact. Any new work that could block the main thread or cause UI jank?
**STOP.** Use `ask_me` once per issue. Do NOT batch. Recommend + WHY. If no issues or fix is obvious, state what you'll do and move on — don't waste a question. Do NOT proceed until user responds.
...
* Analytics. For every new feature: what metric tells you it's working? What tells you it's broken?
* Crash reporting. New crash vectors covered by Crashlytics or equivalent?
...
Evaluate:
* Migration safety. For every data migration: backward-compatible? Non-destructive?
* Feature flags. Should any part be behind a feature flag?
* Rollout order. Correct sequence?
* Rollback plan. Explicit step-by-step.
* Old version and new version running simultaneously — what breaks?
...
Every `ask_me` MUST: (1) present 2-3 concrete options, (2) state which option you recommend FIRST, (3) explain in 1-2 sentences WHY that option over the others, mapping to engineering preferences. No batching multiple issues into one question.

## For Each Issue You Find
* **One issue = one `ask_me` call.** Never combine multiple issues into one question.
...
* **Escape hatch:** If a section has no issues, say so and move on. If an issue has an obvious fix with no real alternatives, state what you'll do and move on — don't waste a question on it. Only use `ask_me` when there is a genuine decision with meaningful tradeoffs.
...
### TODO updates
Present each potential TODO as its own individual `ask_me`. Never batch TODOs — one per question. Never silently skip this step.
...
Identify at least 5 "bonus chunk" opportunities (<30 min each) that would make users think "oh nice, they thought of that." Present each delight opportunity as its own individual `ask_me`. Never batch them. For each one, describe what it is, why it would delight users, and effort estimate.
...
If any `ask_me` goes unanswered, note it here. Never silently default.
